...
export default async function BackgroundRemoverPage({ params }: Props) {
  ...
  const dict = await getDictionary(locale);
  const t = dict.backgroundRemover;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <header className="space-y-2">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600">
            <Eraser size={22} strokeWidth={1.75} />
          </div>
      ...
      <BackgroundRemoverTool dict={t} />
      <AdSenseWrapper slot="background-remover-bottom" className="h-[250px]" />
