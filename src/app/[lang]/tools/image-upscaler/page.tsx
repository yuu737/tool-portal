...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: dict.imageUpscaler.meta.title,
    description: dict.imageUpscaler.meta.description,
    keywords: dict.imageUpscaler.meta.keywords,
    alternates: getAlternates("/tools/image-upscaler", locale),
  };
}
...
export default async function ImageUpscalerPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <header className="space-y-2">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600">
            <ImageUpscale size={22} strokeWidth={1.75} />
          </div>
      ...
      <ImageUpscalerTool dict={t} />
      <AdSenseWrapper slot="image-upscaler-bottom" className="h-[250px]" />
