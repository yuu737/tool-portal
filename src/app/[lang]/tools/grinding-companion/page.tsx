...
import GrindingCompanionTool from "@/components/tools/GrindingCompanionTool";
...
export default async function GrindingCompanionPage({ params }: Props) {
  const { lang } = await params;
  if (!supportedLocales.includes(lang as Locale)) notFound();
  const locale = lang as Locale;
  const dict = await getDictionary(locale);
  const t = dict.grindingCompanion;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <GrindingCompanionTool dict={t} locale={locale} />

      <AdSenseWrapper slot="grinding-companion-bottom" className="h-[250px]" />
